...
## C++ Versions Regression Testing

There are three Python scripts which test an earlier version of the C++ Cstar code against a later version. These scripts all generate test output using the current distribution of Cstar from the parallel distribution directory to the development version running from ../cmake-build-release/cstar.

- `rgc.py` - this program is analogous to the `rgc.sh` opcode comparison script above. It is run with the command

```
% python3 rgc.py
opcode regression complete
```
